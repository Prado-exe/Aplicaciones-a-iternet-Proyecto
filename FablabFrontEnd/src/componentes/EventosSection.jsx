...
const eventos = [
  ...
  { id: 3, titulo: "Arduino Básico", fecha: "28/09/2025", descripcion: "Iniciación práctica a circuitos y Arduino." },
];
...
export default function EventosSection({ handleLinkClick }) {
  ...
  return (
    <section className="relative py-20 bg-gradient-to-b from-gray-950 via-gray-900 to-black text-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid gap-10 md:grid-cols-3">
          {eventos.map((evento, index) => (
            <motion.div
              key={evento.id}
              className="bg-gray-800/50 backdrop-blur-md border border-gray-700/50 rounded-xl p-6 text-center shadow-lg hover:shadow-[0_0_25px_rgba(255,215,0,0.3)] transform hover:scale-[1.03] transition-all duration-500"
              initial={{ opacity: 0, y: 60 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.6, delay: index * 0.2 }}
            >
              <h3 className="text-2xl font-bold text-yellow-300 mb-2">{evento.titulo}</h3>
